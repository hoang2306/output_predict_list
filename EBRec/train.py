...
def main():
    ...
    for lr, l2_reg, item_level_ratio, bundle_level_ratio, bundle_agg_ratio, embedding_size, num_layers, c_lambda, c_temp in \
        ...
        if conf["dataset"]== "Youshu":
            ...
            state_dict_load = torch.load("./savamodel/youshu/GCBRmodel") # edit path 
        ...
        for epoch in range(conf['epochs']):
            ...
            for batch_i, batch in pbar:
                ...
                if (batch_anchor+1) % test_interval_bs == 0:  
                    ...
                    log.close()
                    best_metrics, best_perform, best_epoch = log_metrics(conf, model, metrics, run, log_path, checkpoint_model_path, checkpoint_conf_path, epoch, batch_anchor, best_metrics, best_perform, best_epoch, user_test_list, bundle_test_list, score_test_list)
                    log = open(log_path,"a")
                    log.write("\n")
                    log.close()
...
def log_metrics(conf, model, metrics, run, log_path, checkpoint_model_path, checkpoint_conf_path, epoch, batch_anchor, best_metrics, best_perform, best_epoch, user_list, bundle_list, score_list):
    for topk in conf["topk"]:
